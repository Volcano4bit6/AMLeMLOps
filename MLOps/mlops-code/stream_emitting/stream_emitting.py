...
def main(args):
    for _ in range(20):
        try:
            ...
            producer = KafkaProducer(
                ...
                client_id="trans_stats_producer",
            )

            admin = KafkaAdminClient(
                bootstrap_servers=[args.bootstrap_servers],
                client_id="trans_stats_admin",
            )
    ...
    records = df[['Timestamp', 'From Bank', 'Account', 'To Bank', 'Account.1',
       'Amount Received', 'Receiving Currency', 'Amount Paid',
       'Payment Currency', 'Payment Format', 'Is Laundering']].to_dict("records")
    ...
    while True:
        for record in records:
            record["Timestamp"] = (
                record["Timestamp"] + pd.Timedelta(weeks=iteration)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--topic_name', default="transs",
                        help='Kafka topic name')
    parser.add_argument('-d', '--data', default="data/HI-Small_Trans_stream.parquet",
